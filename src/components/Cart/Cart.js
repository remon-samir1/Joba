...
setProductus(products.filter(data => data.options.rowId != id))
  ...
  return (
    <div className="flex gap-6 mt-6 md:flex-row flex-col justify-center items-start md:justify-start">
                {loading && (
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-3 mt-5 flex-wrap justify-start">
          ...
          products?.map((item,index)=>(
            <div key={index} className="w-[333px] min-w-[300px] rounded-lg overflow-hidden bg-white">
            ...
            <div className="p-4  ">
              ...
              <div className="flex items-center justify-between mt-5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-textColor text-base">{item.instructor.user_name}</span>
                </div>
                <span className="text-base  text-main font-bold">${item.options.discount_price}</span>
              </div>
              <button onClick={()=>handleRemoveItem(item.options.rowId)} className="main-shadow duration-500 flex mt-4 justify-center items-center gap-2 text-white bg-main p-3 rounded w-full">
          {  spinner === item.options.rowId?    <AiOutlineLoading className="load-icon text-white" />
          :<>
  ...
  products.length !== 0 &&
      ...
      <div className="w-[338px] h-max py-5 px-3 rounded bg-white border border-[#dddd]">
        ...
        <div className="py-6 border-b border-[#ddd flex justify-between items-center]">
          ...
          <span className="text-base text-textColor">{values?.discountAmount}</span>
        </div>
        <div className="py-6 border-b border-[#ddd flex justify-between items-center]">
