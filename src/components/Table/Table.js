...
//           ) : item2.key === "buyer" ? (
//             item[item2.key].name
//           ) : item2.key === "coupon_type" ? (
//             item[item2.key] == 1 ? (
//               <span className="text-sm text-white bg-green-600 px-4 py-1 rounded-full">
//                 normal
//               </span>
//             ) : (
//               <span className="text-sm text-white bg-orange-600 whitespace-nowrap px-4 py-1 rounded-full">
//                 one time
//               </span>
//             )
//           ) : (
...
const Table = (props) => {
  ...
  const handleDelete = async (id, parent_id) => {
    ...
    try {
      await Axios.delete(
        `${props.delurl ? props.delurl : props.url}/${id}/${parent_id || ""}`,
        { _method: "DELETE" }
      ).then((data) => {
        setLoadDelete(false);
        props.setDeleted((prev) => !prev);
        if (data.data.status == "error") {
          ...
          toast.success(data.data.status || "Deleted successfully");
        }
      });
    } catch (err) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selectedRows.map((id) =>
          Axios.post(`${props.delurl ? props.delurl : props.url}/${id}`, {
            _method: "DELETE",
          })
        )
      );
  ...
  const showData = props.data?.map((item) => (
    <tr key={item.id}>
      <td>
        <input
          type="checkbox"
            ...
            <th>
              <input
                ...
                onChange={(e) => {
                  ...
                  setSelectedRows(
                    checked ? props.data.map((item) => item.id) : []
                  );
                }}
              />
            </th>
