  ...
  const showData = props.data?.map((item, key)=>(
...

   {
     props.headers.map((item2)=>(
       ...
       {
       item2.key == 'icon' ?
       <img src={`${baseUrl}/${item[item2.key]}`} width="100px" height="30px"/>
       : item2.key == 'show_at_trending' ?  item[item2.key] == 1 ? <span className="text-teal-600 bg-teal-200 py-1 rounded-full px-5">Yes</span> : <span className="text-red-600 bg-red-200 py-1 rounded-full px-5">No</span> :
    
       
       item2.key == 'status' ? <ToggleStatusButton data={item[item2.key]} id={item.id} />
      ...
      {props.action && (
            <td className="flex justify-center items-center gap-3">
              ...
              {props.trash && (
                <Link className="w-7 h-7 bg-red-600 flex justify-center items-center rounded">
                  <Icon
                    icon="mage:trash"
                    width={18}
                    height={18}
