...
const MyCoursesCard = (props) => {
  return (
    <Link
      to={`/student/enrolled-course/${props.slug}`}
      className="bg-white mt-4 border flex flex-col md:flex-row items-center gap-4  border-[#dddd] rounded-lg pl-3 pr-3 md:pr-8 py-3"
    >
      ...
      <div className="flex-1">
        <h4 className="bg-[#FFF2EC]  text-[0.9rem] py-2 px-6 w-max  rounded-full">
          {props.category}
        </h4>
        ...
        <div className="w-full h-1 bg-[#E0E0E0] relative mt-3 rounded-md overflow-hidden">
          <span
            className={`absolute left-0 h-full bg-[#319F43] `}
        ...
        <div className="mt-6 flex items-center">
          ...
          <div className="flex items-center gap-3 px-5 border-x border-x-[#ddd]">
            <Icon
              ...
              height="16"
            />
            <span className="text-[0.7rem] text-text2 ">
