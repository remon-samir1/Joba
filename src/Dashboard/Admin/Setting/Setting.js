...
const Setting = () => {
  ...
  useEffect(() => {
    Axios.get('/admin/social-link').then(data => setForm(data.data.socialLinks.data));
    ...
    <div className="Profile">
      <div className="bg-white p-5 mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
          {Array.isArray(form) && form.map((data, index) => (
            <div className="control" key={index}>
              ...
              <div className="overflow-hidden mt-3 flex items-center border border-borderColor focus-within:border-main rounded gap-3">
                <Icon
                  ...
                  icon={iconsMap[data.name.toLowerCase()] || "grommet-icons:language"}
                  width={20}
                  height={20}
                ...
                <input
                  ...
                  type="text"
                  placeholder={data.name === 'WhatsApp' ? 'Phone number' : 'URL or Username'}
                  id={data.name}
